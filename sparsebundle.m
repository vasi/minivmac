...
- Test
- Write comments
- Find commonality between _read and _write
- Plug into Mini vMac
- Locking
- Don't write zeros
- Read-only opening?
...
bool sparsebundle_read_plist(const char *plist,
                             off_t *band_size,
                             off_t *total_size);
...
ssize_t sparsebundle_read(sparsebundle *sb,
                          void *buf,
                          size_t nbyte,
                          off_t offset);
ssize_t sparsebundle_write(sparsebundle *sb,
                           void *buf,
                           size_t nbyte,
                           off_t offset);

int sparsebundle_open_band(sparsebundle *sb,
                           off_t band_index,
                           int flags,
                           bool *noband);
                             ...
                             off_t *total_size)
{
    ...
    NSString *path = [fileManager
                      stringWithFileSystemRepresentation: plist
                      length: strlen(plist)];
                          ...
                          off_t offset)
{
    ...
    while (cbuf < bufend) {
        ...
        if (fd != -1) {
            ssize_t did_read = pread(fd, cbuf, len, offset);
            close(fd);
            if (did_read == -1) {
                return -1;
            }
                           ...
                           off_t offset)
{
    ...
    while (cbuf < bufend) {
        ...
        int fd = sparsebundle_open_band(sb, band_index, O_WRONLY | O_CREAT,
                                        NULL);
        ...
        ssize_t did_write = pwrite(fd, cbuf, len, offset);
        close(fd);
        if (did_write != len) {
            return -1;
        }
